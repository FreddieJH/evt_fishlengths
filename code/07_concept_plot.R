...
concept_data <-
    ...
    unnest(cols = c(rsample)) |>
    mutate(is_max = rsample == top1) |>
...
pdf_plot <-
    ...
    geom_ribbon(
        aes(ymin = pdf_lwr_under, ymax = pdf_upr_under, fill = model_id),
        alpha = 0.3
    ) +
    geom_line(
        aes(y = pdf_fit_under, col = model_id),
        linewidth = 2,
        lty = "31",
        alpha = 0.5
    ) +
