...
posteriors |>
  ...
  summarise(pdf_median = quantile(pdf, 0.5), .by = c(filename, x)) |>
  ggplot(aes(x, pdf_median, gorup = filename)) +
  geom_path()
...
posteriors |>
  as_tibble() |>
  filter(!is.na(mu)) |>
  filter(.chain == 1, .draw == 1) |>
  expand_grid(x = 1:100) |>
  mutate(
    pdf = pmap_dbl(
      .l = list(x = x, "tnorm", n = lambda, par1 = mu, par2 = sigma),
      .f = g_max
    )
  ) |>
  summarise(pdf_median = quantile(pdf, 0.5), .by = c(filename, x)) |>
