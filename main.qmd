...
p1 <-
    ...
    geom_rect(
        aes(
            xmin = gev_fun(gev_fit, q_pref, "qgev", -1),
            xmax = gev_fun(gev_fit, q_pref, "qgev", 1),
            ymin = -Inf,
            ymax = Inf
        ),
        fill = "grey70",
    ...
    geom_textvline(aes(xintercept = gev_fun(gev_fit, q_pref, "qgev")),
        color = "orange",
        linetype = "dashed",
...
p2 <-
    ...
    geom_vline(
        xintercept = gev_fun(gev_fit, q_pref, "qgev"),
...
qgev_10comps <- gev_fun(gev_fit, 1-(1/10), "qgev")
qgev_100comps <- gev_fun(gev_fit, 1-(1/100), "qgev")
qgev_1000comps <- gev_fun(gev_fit, 1-(1/1000), "qgev")

tibble(x = seq(80, 120, by = 0.1)) %>% 
  mutate(pgev = gev_fun(gev_fit, x, "pgev")) %>% 
  ggplot() +
...
if(!file.exists("output/simulated_data/sim_various_numerical.csv")){
{
    plan(multisession)
    sim_tbl_max <-
        sim_tbl %>%
...
sim_tbl_max %>% 
    unnest(cols = maxima) %>% 
    fwrite("output/simulated_data/sim_various_numerical.csv")
} else {
    sim_tbl_max <- 
    fread("output/simulated_data/sim_various_numerical.csv") %>% 
    nest(.by = -maxima, .key = "maxima")
}
...
estimate_lmax_numerical <- function(maxima, ngrid = 1000, model) {
    {
        stan_code <-
            "data {
...
calc_max(200:250, lambda, mu, sigma)

result <- optimize(calc_max, interval = c(mu - 5*sigma, mu + 5*sigma), 
                   maximum = TRUE, n = n, mu = mu, sigma = sigma)

calc_max(200, 142, 27, 526)
xx <-
    snapper_fit %>%
    as_draws_df() %>%
    as_tibble() %>%
...
xx2 <-
    ...
    as_draws_df() %>%
    as_tibble() %>%
    pivot_longer(contains("max_")) %>% 
    mutate(size = as.numeric(str_extract(name, "\\d+"))) %>% 
    select(-name)
